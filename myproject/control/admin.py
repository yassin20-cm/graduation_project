...
class RequestInline(admin.TabularInline): 
    model = Request
    extra = 0  
    fields = ('request_state', 'request_description', 'request_date')
    readonly_fields = ('request_date',) 

@admin.register(User)
class UserAdmin(admin.ModelAdmin):
    list_display = ('user_id', 'user_name', 'academic_year', 'email', 'join_date','phone_number')
    search_fields = ('user_name', 'email', 'academic_year')
    list_filter = ('academic_year', 'join_date')
    readonly_fields = ('join_date',)
    fieldsets = (
        ('Personal Information', {
            'fields': ('user_name', 'email', 'profile_image', 'academic_year','phone_number')
        }),
        ('Other Details', {
            'fields': ('join_date',)
        }),
